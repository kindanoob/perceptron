...
void Perceptron::InitWindow() {
	...
	window_ = std::move(std::unique_ptr<sf::RenderWindow>(new sf::RenderWindow(sf::VideoMode(kScreenWidthInPixels, 
		kScreenHeightInPixels), kWindowTitle, sf::Style::Default, settings)));
	window_->setVerticalSyncEnabled(true);
}

void Perceptron::InitAxes() {
	std::unique_ptr<sf::Vertex> vertex(new sf::Vertex(sf::Vector2f(0, kOriginY), kAxesColor));
